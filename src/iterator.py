"""Utilities for real-time data augmentation on image data.
   ...
   I changed _get_batches_of_transformed_samples function to add other operations.
   
   If you have any questions, feel free to send an e-mail below.
...
def image_a_b_gen(batch):
    batch = 1.0/255 * batch
    lab_batch = rgb2lab(batch)
    X_batch = lab_batch[:,:,:,0]
    X_batch = X_batch.reshape(X_batch.shape+(1,))
    X_batch = np.concatenate([X_batch, X_batch, X_batch], axis=3)
...
def sparsify_100(y):  # one-hot encoding
    """
    Returns labels in binary NumPy array
    """
    return np.array([[1 if y[i] == j else 0 for j in range(100)]  # 25
                     for i in range(y.shape[0])])
        
def sparsify_25(y):  # one-hot encoding
    """
    Returns labels in binary NumPy array
    """
    return np.array([[1 if y[i] == j else 0 for j in range(25)]  # 25
                     for i in range(y.shape[0])])
