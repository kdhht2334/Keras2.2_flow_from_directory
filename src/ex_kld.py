...
from opt import kullback_leibler_divergence, conv_block, identity_block
...
x = identity_block(x, 3, [24, 64, 64], stage=2, block='c', is_add=True)
...
x = identity_block(x,  3, [64, 64, 64], stage=3, block='d', is_add=True)
...
x = identity_block(x,  3, [64, 96, 96], stage=4, block='f', is_add=True)
...
avg_layer = GlobalAveragePooling2D()(x_b)
model_reg = models.Model(input_img, avg_layer)
...
model_reg.compile(loss=kullback_leibler_divergence,  # Using KL divergence for optimizing model
                  optimizer=sgd,
                  loss_weights=[eta],
                  metrics=['mse'])
...
for epoch in xrange(maxepoches):
    ...
    history = model_reg.fit_generator(
                train_generator3, steps_per_epoch=2000,  #train_generator2.samples // batch_size,
